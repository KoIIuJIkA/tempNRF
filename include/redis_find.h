...
public:
    ...
    std::vector<OptionalString> find(const char* config_path);

    void find_code(json& config_file, 
                   json& profile, 
                   std::vector<json> &match_nfprofiles);

    std::vector<json> find(json nfinstance, json config_file);

    std::vector<json> find(const char* nfinstance_path, 
                           const char* config_path);
};
